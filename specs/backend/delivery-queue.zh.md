...
`delivery_history` 行超过其生效保留窗口即被清扫：以子查询 `LIMIT` 形式的分批 DELETE（`DELETE ... WHERE id IN (SELECT id ... ORDER BY created_at LIMIT $batch)`）循环执行，直到没有剩余行为止。cutoff 按行推导（MRFC 2026-08-31-per-user-history-retention-policy）：用户的 `retention_days` 覆盖全局 `[delivery] history_retention`（默认 `168h`），`0` 永久保存，NULL 继承全局窗口——被用户删除孤立的行（`user_id` 经 ON DELETE SET NULL 置空）始终用全局窗口，因此走 LEFT JOIN。它由 `prune-delivery-history` CLI 子命令（`cmd/prune_delivery_history.go`）运行，与 `prune-expired-posts` 同为按行模式；Ansible playbook 以 `markpost-retention-prune` cron job（`devops/ansible/files/prune-retention.sh`）每日调度两者。历史行只追加，因此该 DELETE 永不与更新竞争。`delivery_attempts` 没有集中清扫 —— 分布式清理见 [`delivery-scheduler.zh.md`](./delivery-scheduler.zh.md)。
