...
**机器账号执笔。** 所有 agent 驱动的 GitHub 操作——认领、push 分支、开 pull request、评论、落栈——以专属机器账号执行（`gh2bda`，本仓库的 collaborator，并被有意设计为可在维护者多个 agent 驱动项目间复用），凭一张带 `repo` 与 `project` scope 的 classic PAT、经 agent 环境的 `GH_TOKEN` 生效，维护者自己的 `gh` 登录原封不动地留给决策动作。classic token 不是仓库级 scope 的——爆炸半径由账号的协作清单而非 token 界定，因此该账号只在运行闭环的仓库获得访问。这组分离买到共享身份买不到的三样东西：审批门禁能区分 agent 预审与人类签核、审计轨迹能把每个时间线动作归因到真实驱动者、agent 的爆炸半径是这些协作仓库而非整个人类账号。
...
闭环的仓库侧在本变更中完整落地：五张 issue 模板与 `config.yml`、pull request 模板、policy 脚本及其挂入 prek 的 32 用例套件、两个 workflow、六个 skill、紧凑的 `AGENTS.md` 章节，以及本记录自身的生命周期移动——bootstrap pull request 顺手演练了它引入的基础 PR 机制。仓库之外的激活已接近完成：机器账号连同其 PAT 与 collaborator 权限就位，个人 Project 带上了 `Loop status` 与 `Priority` 字段，`MARKPOST_PROJECT_TOKEN` secret 已设置，`config.json` 已翻转——剩下的是仅 merge commit 与 v1 分支保护两项维护者设置，在 bootstrap pull request 合并时进行。`type/*` 与 `area/*` 标签已由本变更的 setup 命令建好。首环验证——一个平凡 issue 走快速通道、一个非平凡 issue 走两道门禁——是设计所对照的验收；policy 套件可证明地打挂不合规 pull request、迁移计划器拒绝人类手设状态的回退（两者均由单元测试钉住）。
