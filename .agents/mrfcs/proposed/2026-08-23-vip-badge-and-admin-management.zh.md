...
vip 既已入库（[标志 MRFC](../implemented/2026-08-23-user-vip-flag.zh.md)）、又由策略授予（[授予 MRFC](2026-08-23-github-login-vip-grant-strategy.zh.md)），策略却仍没有面向用户的表面：VIP 用户看不到自己的身份——而这份可见性正是增长策略的全部产品——管理员也没有逐用户杠杆与界面开关。前端在仪表盘欢迎语与应用外壳用户菜单两处渲染当前用户的用户名，管理端在用户列表与详情页渲染用户名；仓库有自己的 Badge 组件与可照抄的治理对话框范式。徽章写什么、出现在哪、管理员如何驱动两个杠杆，就是本层要决定的。
