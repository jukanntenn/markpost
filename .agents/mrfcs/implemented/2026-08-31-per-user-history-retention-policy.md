...
The policy bar's N-days segment shipped as a dead click whenever no positive class default was saved yet: the radio states derived purely from the persisted value and the days input rendered only when one existed, so picking "N days" from Follow global or Forever changed nothing — no input appeared, no write fired (a guard in `commit` also blocked the saved-value fallback). The bar now tracks the selection locally: picking N days reveals and focuses the input, the typed value saves on blur or Enter, and blurring without a committable value drops back to the persisted state. Covered by `VipPolicyBar.test.tsx`.
