...
`delivery_history` rows are swept past their effective retention window by a batched DELETE in the subquery-`LIMIT` form (`DELETE ... WHERE id IN (SELECT id ... ORDER BY created_at LIMIT $batch)`), looped until no rows remain. The cutoff is derived per row (MRFC 2026-08-31-per-user-history-retention-policy): a user's `retention_days` overrides the global `[delivery] history_retention` (default `168h`), `0` keeps forever, NULL inherits the global window — and rows orphaned by user deletion (`user_id` NULL via ON DELETE SET NULL) always use the global window, hence the LEFT JOIN. It runs from the `prune-delivery-history` CLI subcommand (`cmd/prune_delivery_history.go`), the same per-row pattern as `prune-expired-posts`; the Ansible playbook schedules both daily as the `markpost-retention-prune` cron job (`devops/ansible/files/prune-retention.sh`). History rows are append-only, so this DELETE never contends with updates. There is no centralized sweep of `delivery_attempts` — see [`delivery-scheduler.md`](./delivery-scheduler.md) for the distributed cleanup.
