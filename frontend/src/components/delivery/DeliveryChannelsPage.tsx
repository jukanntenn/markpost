...
export function DeliveryChannelsPage() {
  ...
  const toggleMutation = useMutation(
    mutationOptions({
      ...
      onSuccess: (_data, vars) => {
        ...
        toast.success(
          vars.data.enabled ? t("channels.enabledToast") : t("channels.disabledToast"),
        );
      },
    }),
  );
  ...
  return (
    <div className="space-y-6">
      <PageHeading actions={<Button onClick={openNew}><PlusIcon className="mr-1 size-4" />{t("channels.add")}</Button>}>
        {t("channels.title")}
      </PageHeading>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="w-[80px]">{t("channels.colEnabled")}</TableHead>
                  <TableHead>{t("channels.colKeywords")}</TableHead>
                  <TableHead>{t("channels.colLatest")}</TableHead>
                  <TableHead className="w-[120px] text-right">{t("channels.colActions")}</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                    ...
                    <TableRow key={channel.id}>
                      <TableCell>
                        ...
                        <p className="truncate text-xs text-muted-foreground">
                          {truncate(channel.configuration?.webhook_url ?? "", 40)}
                        </p>
                      </TableCell>
                      ...
                      <TableCell>
                        <Switch
                          ...
                          onCheckedChange={(checked) =>
                            toggleMutation.mutate({ id: channel.id, data: { enabled: checked } })
                          }
                        />
                      </TableCell>
                      ...
                      <TableCell>
                        <LatestDeliveryCell latest={latest} loading={latestQuery.isLoading} />
                      </TableCell>
...
function LatestDeliveryCell({
  ...
  if (!latest) {
    return <span className="text-sm text-muted-foreground">{t("latest.never")}</span>;
  }
