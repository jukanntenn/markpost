...
## B2 provisioning (once per environment, before first activation)

1. Create the environment's bucket — `markpost-backups` (production) or `markpost-backups-staging` (staging) — with **versioning enabled** — a compromised host can then only add objects, never overwrite history.
...
| Time (UTC)         | Entry                  | What                                                                      |
| ------------------ | ---------------------- | ------------------------------------------------------------------------- |
| daily 03:30        | `pg-dump-backup.py`    | logical dump → zstd → rclone to B2 `dumps/`, rate-limited to 2 MB/s       |
| daily 04:45        | `pgbackrest-backup.py` | incremental; full on the 1st of the month                                 |
| daily 05:30        | `pgbackrest-check.py`  | archive round-trip check + 26 h freshness + pg_wal growth tripwire → kuma |
| monthly, 2nd 05:00 | `pgbackrest-drill.py`  | restore drill into a throwaway container with row-count assertions        |
...
- **`pgbackrest-check.py` fails on round-trip** — B2 unreachable or credentials broken; watch `pg_wal` growth (the check trips at 128 segments / 2 GB) and fix before the 40 GB disk fills: un-archived segments cannot be recycled and a full disk stops the write path.
