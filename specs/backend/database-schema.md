...
| `VIP`             | `vip`               | boolean                | no       | `false`  | —           | Durable per-user honorific (migration `000008_user_vip`)                                                                                                                                                                   |
| `RetentionDays`   | `retention_days`    | integer                | yes      | —        | —           | Per-user history retention policy (migration `000010_user_retention`): NULL inherits each table's global config, 0 keeps forever, 1–3650 keeps N days — one value drives both the posts and delivery_history prune windows |
| `LastLoginAt`     | `last_login_at`     | timestamp              | yes      | —        | —           | Timestamp of last successful login                                                                                                                                                                                         |
| `TokenVersion`    | `token_version`     | bigint                 | no       | `0`      | —           | Token invalidation epoch for forced logout                                                                                                                                                                                 |
...
Business tables include both `created_at` and `updated_at` with GORM auto-population (`autoCreateTime` / `autoUpdateTime`). Write-once tables (`token_blacklist`) only have `created_at` — their records are never modified after creation. `refresh_tokens` rows are written once at creation and modified only by their soft revocation (`revoked`, `revoked_at`).
