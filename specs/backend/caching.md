...
### Reading origin cache metrics against `CF-Cache-Status`

The origin exports `markpost.render_cache.hit_total`/`miss_total` and the `markpost.cdn.purge_*` counters to the metrics file ([observability.md](./observability.md)); Cloudflare reports the edge's per-response view in `CF-Cache-Status`. The two describe different layers and must not be conflated: the edge absorbs the overwhelming majority of reads (see _Request-flow walkthrough_), so the healthy steady state is a **high origin hit rate over a small origin request volume** — an edge `HIT` never reaches the origin at all. `MISS` or `EXPIRED` means a POP fetched from the origin, and the origin's hit/miss split then decides whether a render happened; `DYNAMIC`/`BYPASS` mark traffic the edge does not cache (the `no-store` API group, authenticated pages). On the deletion side: `purge_skipped_total` climbing alone is the expected self-hosted picture without Cloudflare, while a purge `success` followed by an edge `MISS`/`EXPIRED` on that post's next fetch is the deletion propagating to the edge.
