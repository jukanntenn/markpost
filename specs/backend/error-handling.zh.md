...
- **永不 panic**：error 处理链路保证零 panic，客户端永远收到格式正确的 ErrorResponse（见文末「四层兜底」）。

<a id="the-layered-error-flow"></a>
...
<a id="layer-contracts"></a>

## 分层契约

<a id="the-infra-layer-gorm-error-isolation-bare-pass-through"></a>
...
- **为什么不要 label**：操作上下文（「是在哪个操作出错的」）由 OTel span 的 name 承载（见 [observability.zh.md](./observability.zh.md)）。error 本身只表达「是什么错」，不混入调试文字。已知 sentinel 靠 `errors.Is` 识别，无需 label；意外 error 靠 trace_id → span 调用链定位（比单层 label 更精确）。
...
<a id="the-domain-layer-universal-sentinels"></a>
...
<a id="the-service-layer-minimal-domain-isolation"></a>
...
<a id="the-handler-layer-binding--forwarding"></a>
...
<a id="the-apierr-layer-the-single-entry-point-for-client-error-responses"></a>
...
<a id="the-serviceerror-struct"></a>
...
<a id="the-errcode-struct-carries-its-own-mappings"></a>
...
**无本质缺陷**：指针比较的所谓问题在此设计下不存在——正确的比较范式是按值或按属性，序列化由 `MarshalJSON` 自动处理，apierr 直接用 ErrCode 自带映射。

<a id="error-code-organization-per-domain-files"></a>
...
<a id="shared-error-codes-serviceerrorsgo"></a>
...
| `ErrValidation`     | `validation`      | **422** | 请求参数校验失败（表单 binding）——字段校验失败是「语义上无法处理」（RFC 4918 422），见 [api-design.md](../api-design.zh.md) §3.1 |
...
<a id="domain-specific-code-example-serviceautherrorsgo"></a>
...
<a id="api-error-responses-github-style"></a>
...
<a id="the-frontend-handling-pattern"></a>
...
<a id="the-apierr-package-contract"></a>

## apierr 包契约

`internal/apierr` 是 handler / middleware 返回客户端错误响应的统一入口。

<a id="responderror-logic"></a>
...
<a id="i18n-rendering-with-fallbacks"></a>
...
<a id="where-convenience-functions-live"></a>
...
<a id="the-three-sources-of-binding-errors"></a>
...
<a id="the-unified-entry-handlebindingerror"></a>
...
<a id="registertagnamefunc-replaces-hand-written-reflection"></a>
...
<a id="tagregistry-option-a-explicit-placeholder-mapping"></a>
...
<a id="why-the-validators-built-in-translation-is-not-used"></a>

### 为何不采用 validator 自带 translation
...
<a id="the-paramprovider-mechanism"></a>
...
<a id="middleware-error-handling"></a>

## middleware 错误处理

<a id="panic-recovery-the-fallback-middleware"></a>
...
<a id="rate-limiting-tollbooth"></a>
...
<a id="fault-localization-label-free-trace-based"></a>

## 故障定位（无 label，靠 trace）
...
详见 [observability.zh.md](./observability.zh.md)。

<a id="the-four-fallback-layers-defensive-review-conclusion"></a>
