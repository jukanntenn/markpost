...
test("shows loading state during post key fetch", async ({
  page,
  dashboardPage,
}) => {
  ...
  await expect(
    page.getByText("Loading your post key...", { exact: true })
  ).toBeVisible();
...
test("shows error alert on network abort", async ({ page, dashboardPage }) => {
  await page.route("**/api/post_key", async (route) => {
    await route.abort();
  });
  await page.evaluate(() =>
  ...
  await expect(
    page.getByText("Failed to load post key. Please try again later.")
  ).toBeVisible();
});

test("shows error alert on server error", async ({ page, dashboardPage }) => {
  await page.route("**/api/post_key", async (route) => {
    await route.fulfill({
      status: 500,
      contentType: "application/json",
...
test("uses Accept-Language header with Chinese language", async ({
  ...
  await expect(
    page.getByRole("button", { name: "复制 Post Key", exact: true })
  ).toBeVisible();
});

test("includes Authorization header when fetching post key", async ({
  page,
  dashboardPage,
}) => {
  await page.evaluate(() =>
