...
All traffic in the SaaS topology flows one path: `Client → Cloudflare → Caddy → Go`. Cloudflare sets `CF-Connecting-IP` (the real client IP) and its own `X-Forwarded-For` chain on origin pull; the origin firewall is locked to Cloudflare's CIDRs (see [`cloudflare.md`](./cloudflare.md) _Origin protection_). Client-IP recovery is a single-value relay resting on two aligned trust anchors — Cloudflare's edge assertion of the header, and the host firewall's CIDR allowlist:

**Caddy layer.** Every `reverse_proxy` in `Caddyfile.production.j2` carries `header_up X-Forwarded-For {http.request.header.CF-Connecting-IP}`. Caddy applies user header operations after its default forwarded-header handling, so the `X-Forwarded-For` delivered to Go is always the single `CF-Connecting-IP` value: Cloudflare overwrites any visitor-supplied copy of that header at the edge, and the firewall allows only Cloudflare peers to reach the port, so on every legitimate request the value is Cloudflare-asserted. `trusted_proxies` is set to Cloudflare's published CIDRs on the same blocks, keeping default forwarded-header handling consistent; the XFF value itself is fixed by the `header_up` rewrite.
...
**Cloudflare CIDR maintenance.** The CIDR list is operator-supplied: `cloudflare_cidrs` in `devops/ansible/group_vars/production/vars.yml` feeds `trusted_proxies` (CIDR literals only — Caddy cannot fetch a URL), and the same list is mirrored in the VPS firewall rules. Cloudflare occasionally updates its published ranges (https://www.cloudflare.com/ips/); operators must resync both places — an explicit operational responsibility documented in [`cloudflare.md`](./cloudflare.md).
