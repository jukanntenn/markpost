...
| [dsn.md](./backend/dsn.md) | 数据库连接规范：driver 推断（尽力而为，缺省时从 DSN 推断）、三端 DSN 格式（PostgreSQL pgx keyword/URL + sslmode + Unix socket / MySQL utf8mb4 / SQLite WAL 固定格式）、SQLite 目录自动创建（dirForSQLite 算法 + edge case 表） | <!-- MySQL/SQLite 已移除 -->
