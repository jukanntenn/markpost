...
All traffic in the SaaS topology flows one path: `Client → Cloudflare → Caddy → Go`. Cloudflare rewrites `X-Forwarded-For` and sets `CF-Connecting-IP` on origin pull; the origin firewall is locked to Cloudflare's CIDRs (see [`cloudflare.md`](./cloudflare.md) _Origin protection_). Client-IP recovery uses the standard XFF chain with a two-layer trust configuration, retained as **defense in depth** against the residual threat that the IP allowlist is bypassed via spoofing:

**Caddy layer.** `reverse_proxy` runs with `trusted_proxies` set to Cloudflare's published CIDR ranges. For a request from a Cloudflare peer (`trusted=true`), Caddy appends its own hop to the existing `X-Forwarded-For` that Cloudflare populated with the real client IP. For a request from any other peer (only possible if the allowlist is bypassed), Caddy **overwrites** any client-supplied `X-Forwarded-For` with just the real peer IP — a forged `X-Forwarded-For: 1.2.3.4` is discarded, not appended to. Only a trusted peer may prepend values to the chain.

**gin layer.** `SetTrustedProxies(["127.0.0.1", "::1"])` reflects that Caddy proxies to Go over loopback. `ClientIP()` checks the direct TCP peer, then walks `X-Forwarded-For` right-to-left, skipping trusted-proxy IPs and returning the first untrusted IP — MDN's "trusted proxy list" algorithm. Since legitimate traffic arrives as `<real-client>, <cloudflare-hop>` plus the loopback hop, gin yields the real client IP.

`gin.PlatformCloudflare` (trusting `CF-Connecting-IP` unconditionally) is deliberately not used: it performs no CIDR check, so an attacker who spoofs a Cloudflare source IP past the firewall could also forge that header and evade rate limiting. The XFF design validates the peer at the TCP layer, where forgery is impossible.

**Cloudflare CIDR maintenance.** The CIDR list is operator-supplied in the Caddyfile template (`trusted_proxies` accepts CIDR literals only; it cannot fetch a URL). Cloudflare occasionally updates its published ranges (https://www.cloudflare.com/ips/); operators must resync — an explicit operational responsibility documented in [`cloudflare.md`](./cloudflare.md).
