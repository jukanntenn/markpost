...
Scope first: **the entire relay path — caddy on vps2, frps, frpc — and the observability stack behind it are external services under external operations control; this project deploys none of them and writes none of their config** (review scope rulings on this stack). The repo-owned deliverable converges to the service-integration configuration: markpost's producer-side OTLP settings, plus the interface requirements handed to the external operators — the vhost spec included; every property of the external chain below is something we require and verify, not something we configure.
...
- **Public surface** (all on vps2, all externally configured): caddy `:80`/`:443`, the frps control port `:7000` (token auth, `transport.tls.force = true`), and the loopback binding of frps proxy ports (`proxyBindAddr = "127.0.0.1"`) are interface requirements on the external chain — required and black-box verified from this side, configured by the external operators. The NAS accepts zero inbound connections — frpc only dials out.
- **Two caddy vhosts** (spec handed to the external operators): `grafana.<domain>` for operators (Grafana's own accounts, anonymous access off) and `otlp.<domain>` for producers, with a `remote_ip` allowlist holding producer IPs (vps1 today; one entry per future business system).
...
- Configuration lands with the implementation for the one repo-owned touchpoint: markpost's env vars. The caddy vhost spec, the frpc proxy block, and the frps settings become interface requirements in the handoff document for the external operators — including the requirement that their frpc container joins the shared external docker network so `localIP = "collector"` resolves to the collector by name.
