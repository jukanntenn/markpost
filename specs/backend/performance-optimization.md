...
6. **Tune the Postgres connection pool.** In `internal/infra/db.go` `New`, within the `postgresql` branch only (leave the SQLite `MaxOpenConns(1)` path untouched): <!-- MySQL/SQLite 已移除 -->
   ...
   ```
   *Verification:* unit test (SQLite path) confirms the SQLite `MaxOpenConns(1)` invariant is unchanged; the Postgres path is verified by `go build` and integration. <!-- MySQL/SQLite 已移除 -->
