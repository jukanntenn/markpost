...
Five business counters, attribute-free, one per outcome, live in the OTel metric channel (meter `markpost`, exported via OTLP every 60 s; to `metrics-*.jsonl` only in file mode) alongside the shipped naming and structure — dot-separated semconv-style names (`backend/internal/observability/metrics.go`):
