...
**守护。** 仓库的合并方式为仅 merge commit，v1 分支保护为要求 pull request 加一个 approving review、新推送作废过期批准、要求会话内线程解决、包含管理员、禁止向 `main` force push——两项都是维护者设置步骤，在此记录为闭环的配置。暂不设 required checks：路径过滤的 workflow 对纯 Markdown pull request 整体跳过，一个永不报告的 required check 会卡死每个 RFC 栈；在此期间 checks 汇总由 agent 的 triage 自行核验。补上这道缺口的后续项见下（结论 job）。`deleteBranchOnMerge` 保持 false；验证过的手动清理覆盖部分落地场景。不引入 merge queue——单人维护者没有并发落地压力。
...
闭环的仓库侧在本变更中完整落地：五张 issue 模板与 `config.yml`、pull request 模板、policy 脚本及其挂入 prek 的 32 用例套件、两个 workflow、六个 skill、紧凑的 `AGENTS.md` 章节，以及本记录自身的生命周期移动——bootstrap pull request 顺手演练了它引入的基础 PR 机制。激活在仓库之外完成，每一步都已指名：维护者创建机器账号与其 PAT 并邀为 collaborator；创建带七状态与优先级字段的个人 Project，把 `config.json` 翻转为 `requireProject: true` 并填项目号；把仓库设为仅 merge commit 并按 v1 清单开分支保护；`type/*` 与 `area/*` 标签已由本变更的 setup 命令建好。首环验证——一个平凡 issue 走快速通道、一个非平凡 issue 走两道门禁——是设计所对照的验收；policy 套件可证明地打挂不合规 pull request、迁移计划器拒绝人类手设状态的回退（两者均由单元测试钉住）。
