...
本文档定义 markpost 的数据库连接 DSN 格式规范、driver 推断规则，以及 SQLite 目录自动创建机制。 <!-- MySQL/SQLite 已移除 -->
...
driver 缺省 → 走推断规则 1-6
              → 命中 → 用推断结果
              → 命中 6 兜底 → sqlite（仍算成功） <!-- MySQL/SQLite 已移除 -->
...
请在配置文件中显式指定 [db] driver：
  driver = "sqlite"        # 开发/测试 <!-- MySQL/SQLite 已移除 -->
  driver = "postgresql"    # 生产主选
  driver = "mysql"         # 可选 <!-- MySQL/SQLite 已移除 -->
```
...
markpost 支持 PostgreSQL（生产主选）、MySQL（可选）、SQLite（开发/测试）。三种 driver 的 GORM dialector 选择需要外部输入（`sqlite.Open` / `postgres.New` / `mysql.New` 是不同构造函数）。 <!-- MySQL/SQLite 已移除 -->
...
### 2.2 MySQL <!-- MySQL/SQLite 已移除 -->

底层库：`go-sql-driver/mysql`（gorm-mysql 通过 `mysql.ParseDSN` 解析）。 <!-- MySQL/SQLite 已移除 -->
...
## 三、SQLite 目录自动创建 <!-- MySQL/SQLite 已移除 -->
...
仅当 `driver == "sqlite"`（推断或显式）时执行。mysql/postgres 是远程连接，目录语义不适用。 <!-- MySQL/SQLite 已移除 -->

### 3.3 执行时机

在 `gorm.Open` **之前**，db 初始化阶段（`internal/infra/db.go`）。

### 3.4 解析算法（dirForSQLite） <!-- MySQL/SQLite 已移除 -->

`mattn/go-sqlite3` 驱动的 DSN 解析逻辑（源码确认，`sqlite3.go:1182` 的 `Open` 方法）： <!-- MySQL/SQLite 已移除 -->

- DSN 以 `file:` 开头 → 整串（含 `?query`）透传给 SQLite C 库，由 SQLite 按 [URI 规范](https://www.sqlite.org/uri.html) 解析 <!-- MySQL/SQLite 已移除 -->
- DSN 不以 `file:` 开头 → 驱动自己在第一个 `?` 处截断，把前面的裸路径交给 SQLite <!-- MySQL/SQLite 已移除 -->

驱动**没有**导出"返回纯文件路径"的 helper，须自行复刻这个规则：

```go
func dirForSQLite(dsn string) (dir string, ok bool) { <!-- MySQL/SQLite 已移除 -->
    path := dsn
    ...
    case strings.HasPrefix(path, "file:"):
        ...
        if strings.HasPrefix(body, "//") {
            ...
            filePart := rest[slash:]
            // SQLite 要求 authority 为空或 localhost（否则报错） <!-- MySQL/SQLite 已移除 -->
    ...
    default:
        // 非 file: DSN：裸路径字面传给 SQLite <!-- MySQL/SQLite 已移除 -->
        if path == ":memory:" || path == "" {
            return "", true
        }
        // 误用 sqlite:// scheme（SQLite 不认，会当字面文件名） <!-- MySQL/SQLite 已移除 -->
        if strings.Contains(path, "://") || strings.HasPrefix(path, "sqlite:") { <!-- MySQL/SQLite 已移除 -->
