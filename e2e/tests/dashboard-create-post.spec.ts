...
test("creates a test post via modal and refreshes recent posts", async ({
  ...
  await authenticatedPage
    .getByPlaceholder("Write some Markdown content...")
    .fill("Hello world body");
  ...
  await expect(authenticatedPage.getByText("Post Created")).toBeVisible({
    timeout: 10000,
  });
...
test("disables Create when body is empty", async ({ authenticatedPage, dashboardPage }) => {
  await dashboardPage.goto();
  await expect(dashboardPage.postKeyHeading).toBeVisible({ timeout: 15000 });
