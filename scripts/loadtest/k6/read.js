...
import { originGet, bandwidthSummary, revalidate304, coldMiss200, baseURL, tlsOptions, summaryTrendStats } from './lib.js';
...
function thresholdsFor(s) {
  if (s === 'cold-miss') {
    ...
    return { http_req_duration: ['p(95)<1500'], http_req_failed: ['rate<0.02'] };
  }
  if (s === 'revalidate-304') {
...
export function handleSummary(data) {
  ...
  const r304 = (data.metrics.origin_revalidate_304 && data.metrics.origin_revalidate_304.values.count) || 0;
  const c200 = (data.metrics.origin_cold_miss_200 && data.metrics.origin_cold_miss_200.values.count) || 0;
